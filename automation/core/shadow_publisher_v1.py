...
EXPORT_ROOT = PLATFORM / "exports" / "flutter_shadow"
PUBLISH_ROOT = PLATFORM / "publish_ready" / "flutter_shadow"
REPORTS = PLATFORM / "reports"

LATEST_EXPORT = EXPORT_ROOT / "latest_shadow_export.json"
PUBLISH_REPORT = REPORTS / "shadow_publisher_v1_report.json"
...
def validate_manifest(package_dir, manifest):
    ...
    if manifest.get("mode") != "SHADOW_READ_ONLY":
        problems.append("manifest mode SHADOW_READ_ONLY degil")
    ...
    if policy.get("shadowOnly") is not True:
        problems.append("shadowOnly true degil")
...
def main():
    ...
    if not package_id or not manifest_rel or not expected_manifest_sha:
        return fail(
            ...
            "latest_shadow_export.json eksik alan iceriyor.",
            latest,
        )
